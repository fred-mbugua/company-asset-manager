 ...
 */
function buildAccessFilter(AccessFilterContext, startParamIndex = 1, requireBranchJoin = false) {
    const conditions = [];
    const values = [];
...
class AssetModel {
    ...
    static async count(AccessFilterContext) {
    ...
    static async findAll(page = 1, itemsPerPage = 20, AccessFilterContext) {
        ...
        const [result, countResult] = await Promise.all([
            database_1.default.query(query, [...values, itemsPerPage, offset]),
            database_1.default.query(countQuery, values)
        ]);
        return {
            assets: result.rows,
    ...
    static async search(filters, AccessFilterContext) {
        const queryParams = [];
        const whereClauses = [];
        let paramIndex = 1;
    ...
    static async findAllFiltered(filters, AccessFilterContext) {
        const queryParams = [];
        const whereClauses = [];
        let paramIndex = 1;
        // Add access filter first

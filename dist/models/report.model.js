 ...
 */
function buildAccessFilterConditions(filter, config = {}, startParamIndex = 1) {
    const conditions = [];
    const values = [];
 ...
 */
function createAccessFilter(AccessFilterContext) {
...
class ReportModel {
     ...
     */
    async getTotalAssetValue(AccessFilterContext) {
    ...
    async getTotalValueByCategory(assetType, AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        ...
        if (whereClause)
            conditions.unshift(whereClause);
        const query = `
      SELECT COALESCE(SUM(purchase_price), 0) AS total_value 
      FROM assets
      ${branchJoin}
    ...
    `;
        const result = await database_1.default.query(query, [...values, assetType]);
        return result.rows[0].total_value;
    }
    /**
     * Get total asset count with optional access filtering
     */
    async getTotalAssetCount(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        const branchJoin = needsBranchJoin ? 'INNER JOIN branches ON assets.branch_id = branches.id' : '';
        const query = `
     ...
     */
    async getTotalExpenseSum(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
     ...
     */
    async getExpenseDetailForAllAssets(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
     ...
     */
    async getAssetsByType(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        const branchJoin = needsBranchJoin ? 'INNER JOIN branches ON assets.branch_id = branches.id' : '';
        const query = `
     ...
     */
    async getAssetsByStatus(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        const branchJoin = needsBranchJoin ? 'INNER JOIN branches ON assets.branch_id = branches.id' : '';
        const query = `
     ...
     */
    async getAssetsByBranch(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
     ...
     */
    async getMonthlyExpenses(months = 6, AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        const branchJoin = needsBranchJoin ? 'INNER JOIN branches ON assets.branch_id = branches.id' : '';
     ...
     */
    async getRecentAssignments(limit = 10, AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, nextParamIndex, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
    ...
    `;
        const result = await database_1.default.query(query, [...values, limit]);
        return result.rows;
    }
    /**
     * Get top expensive assets with optional access filtering
     */
    async getTopExpensiveAssets(limit = 5, AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, nextParamIndex, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
     ...
     */
    async getAssignmentStats(AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        const branchJoin = needsBranchJoin ? 'INNER JOIN branches ON assets.branch_id = branches.id' : '';
        const query = `
     ...
     */
    async getRecentExpenses(limit = 10, AccessFilterContext) {
        const filter = createAccessFilter(AccessFilterContext);
        const { whereClause, values, nextParamIndex, needsBranchJoin } = buildAccessFilterConditions(filter, { branchTable: 'assets' });
        // Use INNER JOIN when company filtering is needed
        const branchJoinType = needsBranchJoin ? 'INNER JOIN' : 'LEFT JOIN';

...
/**
 * Access filter context containing user's access restrictions
 */
...
/**
 * Query filter result for SQL queries
 */
...
class AccessFilterUtil {
  ...
  async buildContext(
    user: any,
    permissionContext?: { branchLevelAccess: boolean; userBranchId: number | null }
  ): Promise<AccessFilterContext> {
    const isAdmin = user?.role === 'Admin';
    const branchLevelAccess = permissionContext?.branchLevelAccess || false;
    ...
    const context: AccessFilterContext = {
      userId: user?.id,
      roleId: user?.role_id,
      branchId: user?.branch_id || permissionContext?.userBranchId || null,
   ...
   */
  private async checkCompanyLevelAccess(roleId: number): Promise<boolean> {
    ...
    try {
      ...
      return result.rows[0]?.has_company_access || false;
    } catch (error) {
  ...
  async getAccessibleBranchIds(userBranchId: number): Promise<number[]> {
    try {
      return await BranchModel.getAccessibleBranchIds(userBranchId);
    } catch (error) {
   ...
   */
  async getAccessibleCompanyIds(roleId: number, userCompanyId: number | null): Promise<number[]> {
    try {
      ...
      `;
      const restrictionResult = await pool.query(restrictionQuery, [roleId]);
   ...
   */
  buildSQLFilter(
    context: AccessFilterContext,
    tableConfig: {
      branchTable?: string;
      branchField?: string;
      companyTable?: string;
      companyField?: string;
      employeeCompanyField?: string;
    } = {},
   ...
   */
  buildWhereClause(filter: QueryFilter, existingConditions: string[] = []): string {
   ...
   */
  async getAccessibleCompanyNames(companyIds: number[]): Promise<string[]> {
   ...
   */
  async canAccessBranch(context: AccessFilterContext, targetBranchId: number): Promise<boolean> {
    if (context.isAdmin) return true;
    if (!context.branchLevelAccess) return true;
    if (!context.accessibleBranchIds) return true;
   ...
   */
  async canAccessCompany(context: AccessFilterContext, targetCompanyId: number): Promise<boolean> {
    if (context.isAdmin) return true;
    if (!context.companyLevelAccess) return true;
    if (!context.accessibleCompanyIds) return true;
    
    return context.accessibleCompanyIds.includes(targetCompanyId);
  }
}

export default new AccessFilterUtil();
